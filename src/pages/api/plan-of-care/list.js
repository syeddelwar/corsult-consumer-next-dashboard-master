  ...
  switch (method) {
    case "GET":
      try {
        ...
        let planOfCares = await PlanOfCareModel.find()
          .sort({ createdAt: -1 })
          .limit(10)
          .populate("consumer_contract_id");

        if (query) {
          const planOfCaresCursor = await PlanOfCareModel.aggregate([
            {
              $lookup: {
          ...
          planOfCares = planOfCaresCursor;
        }
        return res.json(planOfCares);
