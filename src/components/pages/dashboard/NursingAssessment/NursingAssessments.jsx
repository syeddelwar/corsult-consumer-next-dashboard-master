...
const NursingAssessments = () => {
  // Custom Hooks
  const { list } = useNursingAssessment();
  ...
  const pageContent = () => {
    if (assessments.length > 0) {
      return assessments.map((assessment, i) => {
        return (
          <tr className="bg-white border-b" key={assessment._id}>
            <th className="px-6 py-4">{i + 1}</th>
            <th className="px-6 py-4">{assessment.createdAt.substr(0, 10)}</th>

            <td className="px-6 py-4">
              {`${assessment.consumer_contract_id?.consumerInfo?.consumer_first_name} ${assessment.consumer_contract_id?.consumerInfo?.consumer_last_name}`}
            </td>
            <td className="px-6 py-4">
              {
                assessment.consumer_contract_id?.consumerInfo
                  ?.consumer_email_address
              }
            </td>
            <td className="px-6 py-4">
              {assessment.consumer_contract_id?.consumerInfo?.consumer_cell}
            </td>
            <td className="px-6 py-4">
            ...
            <td className="px-6 py-4 flex items-center gap-2">
              <Link
                href={`${routes.generatePDF}&app_id=${assessment.consumer_contract_id?._id}`}
              >
                <IconPlain
              ...
              <Link
                href={`${routes.nursingAssessmentRead}/${assessment.consumer_contract_id?._id}`}
              >
                <IconPlain
